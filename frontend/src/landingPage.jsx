...
export default function LandingPage() {
  ...
  // 🔹 Ambil konten landing page dari backend
  useEffect(() => {
  ...
  return (
    <div
      ...
      style={{ fontFamily: hero.font || "Inter" }}
    >
      {/* === NAVBAR === */}
      <nav
      ...
      >
        <div className="max-w-6xl mx-auto px-5 sm:px-8 flex justify-between items-center">
          ...
          <div className="hidden md:flex gap-8 items-center font-medium text-gray-700">
            <button onClick={() => scrollToSection(berandaRef)} className="hover:text-orange-500">Beranda</button>
            <button onClick={() => scrollToSection(fasilitasRef)} className="hover:text-orange-500">Fasilitas</button>
            <button onClick={() => scrollToSection(kontakRef)} className="hover:text-orange-500">Kontak</button>
        ...
        <div className={`md:hidden bg-white/90 backdrop-blur-md overflow-hidden transition-all duration-500 ${open ? "max-h-80 py-5" : "max-h-0"}`}>
          <div className="flex flex-col items-center gap-3 text-gray-700 font-medium text-sm">
            <button onClick={() => scrollToSection(berandaRef)} className="hover:text-orange-600">Beranda</button>
            <button onClick={() => scrollToSection(fasilitasRef)} className="hover:text-orange-600">Fasilitas</button>
            <button onClick={() => scrollToSection(kontakRef)} className="hover:text-orange-600">Kontak</button>
          ...
          <div className="flex-1 text-center md:text-left space-y-6">
            ...
            <div className="inline-block mt-4">
              <Link
                ...
                className="inline-flex items-center px-6 py-3 rounded-full bg-orange-600 text-white font-semibold shadow-md hover:shadow-xl hover:-translate-y-[2px] transition"
              >
      ...
      {/* === FASILITAS === */}
      <section ref={fasilitasRef} className="bg-white py-16 text-center px-6 md:px-12 lg:px-20">
        ...
        <div className="grid gap-10 sm:grid-cols-2 md:grid-cols-3">
          {[fasilitas1, fasilitas2, fasilitas3].map((img, i) => (
            <div key={i} className="bg-gradient-to-br from-orange-50 via-yellow-50 to-white rounded-3xl shadow-md hover:shadow-xl transition-all p-6 flex flex-col items-center">
              <img src={img} alt={`Fasilitas ${i + 1}`} className="w-full h-[200px] object-cover rounded-2xl mb-5" />
              ...
              <Link to="/detail-layanan" className="bg-gradient-to-r from-orange-500 to-yellow-400 text-white px-6 py-2 rounded-full text-sm hover:scale-105 transition">
                Lihat Detail
              </Link>
            </div>
          ))}
      ...
      {/* === KONTAK === */}
      <section ref={kontakRef} className="relative bg-gradient-to-r from-orange-400 to-yellow-400 py-16 px-6 md:px-16 flex flex-col md:flex-row items-center justify-between gap-10 text-white overflow-hidden">
        ...
        <div className="md:w-1/2 flex justify-center md:justify-end">
          <img src={kontak.image || kontak1} alt="Kontak" className="w-full max-w-xs sm:max-w-sm rounded-3xl drop-shadow-2xl hover:scale-105 transition duration-500" />
        </div>
      </section>
      ...
      <footer className="bg-white pt-14 pb-8 px-6 md:px-16 border-t border-gray-200 mt-16 text-center md:text-left text-[13px] text-gray-600">
        <div className="grid gap-8 md:grid-cols-3">
          ...
          <div>
            ...
            <button onClick={() => scrollToSection(berandaRef)} className="block hover:text-orange-600">Beranda</button>
            <button onClick={() => scrollToSection(fasilitasRef)} className="block hover:text-orange-600">Fasilitas</button>
            <button onClick={() => scrollToSection(kontakRef)} className="block hover:text-orange-600">Kontak</button>
          </div>
        </div>
